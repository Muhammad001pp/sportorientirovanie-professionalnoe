...
function JudgesList({ adminKey, filter }: { adminKey: string; filter: "all" | "pending" | "approved" | "rejected" }) {
  const statusArg = filter === "all" ? undefined : filter;
  const list = useQuery(api.judges.listJudges as any, { adminKey, status: statusArg } as any) ?? [];
  const setStatus = useMutation(api.judges.setJudgeStatus);
  if (!list.length) return <p>Нет заявок</p>;
  return (
    <ul>
      {list.map((j: any) => (
        <li key={j._id} style={{ marginBottom: 12 }}>
          <b>{j.publicNick}</b> — {j.fullName} — {j.email} — {j.phone} — <i>{j.status}</i>
          <div style={{ display: "inline-flex", gap: 8, marginLeft: 12 }}>
            {j.status !== "approved" && (
              <button onClick={() => setStatus({ adminKey, judgeId: j._id, status: "approved" })}>Одобрить</button>
            )}
            {j.status !== "rejected" && (
              <button onClick={() => setStatus({ adminKey, judgeId: j._id, status: "rejected" })}>Отклонить</button>
            )}
            {j.status !== "pending" && (
              <button onClick={() => setStatus({ adminKey, judgeId: j._id, status: "pending" })}>Вернуть в ожидание</button>
            )}
          </div>
        </li>
      ))}
